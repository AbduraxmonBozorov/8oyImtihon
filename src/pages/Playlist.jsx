...
export default function PlaylistView() {
  ...
  const { currentPlaylist, likedSongs, accessToken, currentTrack, isPlaying } =
  ...
  useEffect(() => {
    if (currentPlaylist && currentPlaylist.tracks) {
      dispatch(
        setPlaylist(currentPlaylist.tracks.items.map((item) => item.track))
      );
    }
  ...
  if (!currentPlaylist || !currentPlaylist.tracks) {
    return <div className="text-white text-center p-8">xatolik yuz berdi.</div>;
  }
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="font-semibold">
              {currentPlaylist.owner?.display_name} tomonidan
            </span>
            <span>•</span>
            <span className="text-gray-200">
              {tracks.length} qo'shiq, {formatDuration(totalDuration)}
            </span>
          </div>
        </div>
      </div>
              ...
              return (
                <tr
                  ...
                  className={`hover:bg-white/10 group rounded-md transition-colors ${
                    isCurrentlyPlaying ? "bg-white/20" : ""
                  }`}
                >
                  <td className="py-5 w-12">
                    ...
                    <span
                      className={`visible group-hover:invisible ${
                        isCurrentlyPlaying ? "text-[#1ed760]" : ""
                      }`}
                    >
                      {index + 1}
                    </span>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      <img
                        src={
                          song.album.images[0]?.url || "/api/placeholder/40/40"
                        }
                        alt={song.name}
                        className={`w-10 h-10 ${
                          isCurrentlyPlaying ? "animate-spin" : ""
                        }`}
                      />
                      <div>
                        <div
